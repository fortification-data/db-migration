...
DONT_NEED_CLEANING = ["ff_opportunity", "summary_nutrient_intake", "nutrition_status"]
...
def clean_health_impact():
    ...
    df = df[(df["redcap_event_name"].str.endswith("arm_1")) & (df["redcap_event_name"] != "all_foods_arm_1")].copy()
...
def clean_foundational_documents_review():
    ...
    df[(df["redcap_event_name"].str.endswith("arm_1") & (df["redcap_event_name"] != "all_foods_arm_1"))].to_csv(
        "../data/cleaned/foundational_documents_review.csv", index=False
    )
...
def clean_coverage():
    for coverage_file in Path("../data/redcap").glob("coverage_*.csv"):
        ...
        df = df[(df["redcap_event_name"] != "all_foods_arm_1") & (df["redcap_event_name"].str.endswith("arm_1"))].copy()
        df.to_csv(
            f"../data/cleaned/{coverage_file.stem}.csv", index=False
        )


def clean_intake():
    df = pd.read_csv("../data/redcap/intake.csv", low_memory=False)
    df = df[(df["redcap_event_name"] == "all_foods_arm_1") & (df["redcap_repeat_instrument"] == "intake")].copy()
...
def clean_nutrients_compounds():
    ...
    df = df[(df["redcap_repeat_instrument"] == "nutrients_compounds") & (df["redcap_event_name"].str.endswith("arm_1") & (df["redcap_event_name"] != "all_foods_arm_1"))].copy()
...
if __name__ == "__main__":
    ...
    source_files = len(list(Path("../data/redcap").glob("*.csv"))) - len(DONT_NEED_CLEANING)
